...
class NicheQuery:
    ...
    def query(
        self,
        query_adata_list,
        query_niche_embedding,
        query_result_key,
        niche_embedding_key,
    ):
        for adata in query_adata_list:
            assert niche_embedding_key in adata.obsm.keys(), (
                f"{niche_embedding_key} not in adata"
            )
        adata_list = []
        for adata in query_adata_list:
            cos_sim = cosine_similarity(
                query_niche_embedding.reshape(1, -1), adata.obsm[niche_embedding_key]
            )
