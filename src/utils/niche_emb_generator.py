...
class NicheEmbGenerator:
    ...
    def generate_niche_embedding(self, mask, embedding_key, adata):
        assert embedding_key in adata.obsm.keys(), "Invalid embedding key: embedding not exists in adata"
        return adata.obsm[embedding_key][mask].mean(axis=0)

    # generate niche embedding for all nodes
    def generate_niche_embeddings(self, niche_embedding_key, embedding_key, coordinates_key, k, adata_list):
        assert 0 < k <= 10, "Invalid subgraph parameter, should be between 1 and 10 (included)"
        for adata in adata_list:
            assert coordinates_key in adata.obsm.keys(), "Invalid spatial coordinates key"
            assert embedding_key in adata.obsm.keys(), "Invalid embedding key: embedding not exists in adata"
        for adata in adata_list:
            ...
            adata.obsm[niche_embedding_key] = np.asarray(niche_connectivity_norm @ adata.obsm[embedding_key])
