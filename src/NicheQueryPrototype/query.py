...
class NicheQuery:
    ...
    def niche_query(self, sample: Sample) -> np.ndarray:
    ...
    """
    Perform niche query task and return samples with similar niches.
    ...
    def niche_query(self, db: Database, niche: Niche):
        target_samples = [s for s in db.samples if s.target_parcellation_mask.sum() > 0]
        non_target_samples = [s for s in db.samples if s.target_parcellation_mask.sum() == 0]
        ...
        best_threshold = self.get_best_threshold(val_samples)
        ...
        logger.info(f"Classification report with {len(all_test_samples)} samples:")
        logger.info(classification_report(y_true=parcellation_mask, y_pred=(cos_sim > best_threshold)))
